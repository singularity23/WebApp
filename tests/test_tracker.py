...
from todo.models import Hazard, Comment
...
def test_tracker_task_creation(todo_setup, django_user_model):
    ...
    # test task creation
    task_count = Hazard.objects.count()
    consumer([msg])

    assert task_count + 1 == Hazard.objects.count(), "task wasn't created"
    task = Hazard.objects.filter(title="[TEST] test1 subject").first()
    ...
    task_count = Hazard.objects.count()
    consumer([msg])
    assert task_count == Hazard.objects.count(), "comment created another task"
    Comment.objects.get(
