...
from todo.models import Comment, Hazard, Project
...
def parse_references(task_list, references):
    ...
    for related_message in references.split():
        ...
        new_answer_thread = Hazard.objects.filter(task_list=task_list, pk=thread_id).first()
...
def insert_message(task_list, message, priority, task_title_format):
    ...
    best_task = (
        Hazard.objects.filter(task_list=task_list, comment__email_message_id__in=related_messages)
    ...
    with transaction.atomic():
        if best_task is None:
            best_task = Hazard.objects.create(
                priority=priority,
...
def tracker_consumer(
    ...
    task_list = Project.objects.get(group__name=group, slug=task_list_slug)
